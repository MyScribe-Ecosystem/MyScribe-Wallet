...
import { useAutoLockTimeId, useWalletHealthDelayId } from '@/ui/state/settings/hooks';
...
export default function AdvancedScreen() {
    ...
    const [walletHealthDelayPopoverVisible, setWalletHealthDelayPopoverVisible] = useState(false);
        ...
        <Layout>
            ...
            <div
                ...
                <div
                    ...
                    {/* Wallet Health Popup Delay Setting */}
                    <div
                        style={{
                            display: 'flex',
                            alignItems: 'center',
                        ...
                        onClick={() => setWalletHealthDelayPopoverVisible(true)}
                        onMouseEnter={(e) => {
                            e.currentTarget.style.background = colors.buttonHoverBg;
                        }}
                        onMouseLeave={(e) => {
                        ...
                        <div
                            ...
                            <MedicineBoxOutlined style={{ fontSize: 18, color: colors.main }} />
                        </div>
                        ...
                        <div style={{ flex: 1 }}>
                            ...
                            <div
                                ...
                                Delay before displaying wallet health popup again
                            </div>
                        </div>
            ...
            {walletHealthDelayPopoverVisible ? (
                <WalletHealthDelayPopover
                    onNext={() => setWalletHealthDelayPopoverVisible(false)}
                    onCancel={() => setWalletHealthDelayPopoverVisible(false)}
                />
            ) : null}
...
export const WalletHealthDelayPopover = ({ onNext, onCancel }: { onNext: () => void; onCancel: () => void }) => {
    const [selectedId, setSelectedId] = useState<number | null>(null);
    const [isProcessing, setIsProcessing] = useState(false);
    const walletHealthDelayId = useWalletHealthDelayId();
    const dispatch = useAppDispatch();
            ...
            <Column
                ...
                <div
                    ...
                    <div
                        ...
                        Choose when to receive Wallet Health Popup
                    </div>
                </div>
                ...
                <div
                    ...
                    {WALLET_HEALTH_DELAYS.map((option, index) => {
                        const isSelected = option.id === walletHealthDelayId;
                        const isHovered = option.id === selectedId;
                        ...
                        return (
                            <div
                                ...
                                onClick={async () => {
                                    if (option.id === walletHealthDelayId || isProcessing) return;

                                    setIsProcessing(true);
                                    try {
                                        await wallet.setWalletHealthDelayId(option.id);
                                        dispatch(settingsActions.updateSettings({ walletHealthDelayId: option.id }));
                                        tools.toastSuccess(`Wallet Health delay changed to ${option.label}`);
                                        onNext();
                                    } catch (error) {
                                        tools.toastError('Failed to update Wallet Health delay');
                                    } finally {
                                        setIsProcessing(false);
                                    }
                                }}
